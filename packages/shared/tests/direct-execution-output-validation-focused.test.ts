// tests/direct-execution-output-validation-focused.test.ts - Focused working output validation tests 
...
describe('DirectExecutionProvider Output Validation - Working Tests', () => {
  ...
  describe('Non-JSON Output Handling', () => {
    ...
    it('should handle multiline text output using node', async () => {
      const tool: EnactTool = {
        name: 'test/multiline-output',
        description: 'Tool that outputs multiple lines',
        command: 'node -e "console.log(\\"Line 1\\"); console.log(\\"Line 2\\"); console.log(\\"Line 3\\");"',
        timeout: '5s'
      };
  ...
  describe('Error Output Validation (Shell-based)', () => {
    it('should handle stderr output correctly with shell wrapper', async () => {
      const tool: EnactTool = {
  ...
  describe('Environment Variable Output Effects (Shell-based)', () => {
  ...
  describe('Platform-Specific Output Tests', () => {
    it('should handle long line output with simpler approach', async () => {
      const tool: EnactTool = {
        name: 'test/long-line',
        description: 'Tool with very long single line',
        command: 'node -e "console.log(\\"🎉\\".repeat(5000))"',
        timeout: '10s'
      };
    ...
    it('should handle control characters in a simple way', async () => {
      const tool: EnactTool = {
        name: 'test/control-chars',
        description: 'Tool with control characters in output',
        command: 'node -e "process.stdout.write(\\"Before\\\\r\\\\nReturn\\\\tTab\\")"',
        timeout: '5s'
      };
      ...
      expect(result.output.stdout).toBeDefined();
      expect(typeof result.output.stdout).toBe('string');
      expect(result.output.stdout).toContain('Before');
    });

    it('should handle output with only whitespace using simpler command', async () => {
      const tool: EnactTool = {
        name: 'test/whitespace-output',
        description: 'Tool that outputs only whitespace',
        command: 'node -e "console.log(\\"   \\"); console.log(\\"\\t\\"); console.log(\\"   \\");"',
        timeout: '5s'
      };
