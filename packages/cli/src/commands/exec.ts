...
import { parseTimeout } from "@enactprotocol/shared/utils";
import { DirectExecutionProvider } from "@enactprotocol/shared/core";
import { DaggerExecutionProvider } from "@enactprotocol/shared/core";
...
async function executeCommand(
	...
	return executionProvider.executeCommandExecStyle(
		command,
		timeout,
		verbose,
		envVars,
...
export function configureExecution(config: ExecConfig): void {
	...
	if (config.provider) {
		console.log(
			pc.cyan(`🔧 Switching to ${config.provider} execution provider`),
		);
