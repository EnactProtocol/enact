...
import { createEnactApiClient } from "@enactprotocol/shared/api";
import { getDefaultUrl } from "@enactprotocol/shared/utils";
...
export async function handleUserCommand(
	...
	switch (subcommand) {
		...
		default:
			if (!subcommand) {
				console.error(
					pc.red("Missing subcommand. Use --help for usage information."),
				);
			} else {
...
async function handlePublicKeyCommand(
	args: string[],
	options: UserCommandOptions,
) {
	...
	try {
		...
		p.outro(
			pc.red(
				`✗ Authentication required. Run "enact auth login" to authenticate.`,
			),
		);
		return;
	}
	...
	try {
		...
		if (options.format === "json") {
			...
			p.note(
				`User ID: ${pc.cyan(userId)}\nPublic Key: ${pc.yellow(data.publicKey || "Not available")}\nKey Type: ${pc.blue(data.keyType || "Unknown")}\nCreated: ${pc.gray(data.createdAt ? new Date(data.createdAt).toLocaleString() : "Unknown")}`,
		...
		if (error instanceof EnactApiError) {
			if (error.statusCode === 401 || error.statusCode === 403) {
				...
				p.note(
					`User '${userId}' not found or no public key available.`,
					"Error",
				);
			...
			if (
				...
				p.note(
					"Could not connect to the Enact API. Check your internet connection and server URL.",
					"Error",
				);
			} else {
				p.note(error.message, "Error");
