...
`);
  ...
  if (!toolName) {
    ...
    toolName = nameResponse as string;
  }
  ...
  if (options.minimal) {
    ...
    p.outro(pc.green(`✓ Created minimal tool definition: ${pc.bold(fileName)}`));
    return;
  }
  ...
  while (addMore) {
    ...
    if (paramType === 'string') {
      ...
      if (addEnum) {
        ...
        if (enumValues) {
          properties[paramName].enum = enumValues.split(',').map(v => v.trim());
        }
      }
    }
    ...
    if (isRequired) {
      ...
      if (defaultValue) {
        properties[paramName].default = paramType === 'integer' || paramType === 'number' 
          ? Number(defaultValue) 
          : paramType === 'boolean' 
    ...
    if (inputSchema && inputSchema.properties) {
      ...
      for (const [key, prop] of Object.entries(inputSchema.properties) as [string, any][]) {
        const value = await p.text({
          ...
          placeholder: prop.default || (prop.type === 'string' ? 'example-value' : '0')
        }) as string;
        
        if (value) {
          input[key] = prop.type === 'integer' || prop.type === 'number'
            ? Number(value)
            : prop.type === 'boolean'
 ...
 */
function generateMinimalYaml(name: string, description: string, command: string): string {
  return `name: ${name}
...
`;
  ...
  if (config.examples) {
    yaml += `\n# Test cases\nexamples:\n`;
    yaml += config.examples.map((ex: any) => {
