...
async function detectMcpClients(): Promise<Array<{id: string, name: string, configPath: string}>> {
  const currentPlatform = platform() as keyof typeof MCP_CLIENTS['claude-desktop']['configPaths'];
  const detected = [];
...
async function installMcpServer(client: {id: string, name: string, configPath: string}): Promise<void> {
  if (client.id === 'goose') {
    ...
    try {
      ...
      throw new Error(`Failed to configure Goose extension: ${(error as Error).message}`);
    }
    return;
  }
...
async function checkMcpServerInstalled(client: {id: string, name: string, configPath: string}): Promise<boolean> {
  if (client.id === 'goose') {
    try {
      const yaml = await import('yaml');
