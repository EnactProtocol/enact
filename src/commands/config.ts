 ...
 */
async function saveConfig(config: EnactConfig, global: boolean = false): Promise<void> {
  const configPath = getConfigPath(global);
 ...
 */
async function setConfigValue(key: string, value: string, global: boolean = false): Promise<void> {
 ...
 */
async function getConfigValue(key: string, global: boolean = false): Promise<void> {
  const config = await loadConfig(global);
...
async function interactiveConfig(global: boolean = false): Promise<void> {
  ...
  const verificationPolicy = await p.select({
    ...
    options: [
      { value: 'permissive', label: 'Permissive (allow unsigned tools with warning)' },
  ...
  const newConfig: EnactConfig = {
    ...
    verificationPolicy: verificationPolicy as 'permissive' | 'enterprise' | 'paranoid',
  ...
  if (executionProvider === 'dagger') {
    ...
    console.log(pc.gray('   curl -L https://dl.dagger.io/dagger/install.sh | sh'));
 ...
 */
export async function handleConfigCommand(args: string[], options: ConfigOptions): Promise<void> {
  if (options.help) {
    showConfigHelp();
    return;
  }
 ...
 */
export async function getCurrentConfig(global: boolean = false): Promise<EnactConfig> {
