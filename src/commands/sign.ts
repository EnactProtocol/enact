...
import { verifyTool, VERIFICATION_POLICIES, getTrustedPublicKeysMap, signTool } from '../security/sign';
 ...
 */
async function handleVerifyCommand(args: string[], options: SignOptions): Promise<void> {
  let toolPath = args[0];
  ...
  const policyKey = policyName.toUpperCase() as keyof typeof VERIFICATION_POLICIES;
 ...
 */
async function handleListKeysCommand(args: string[], options: SignOptions): Promise<void> {
 ...
 */
async function handleSignToolCommand(args: string[], options: SignOptions): Promise<void> {
  let toolPath = args[0];
  ...
  if (!privateKeyPath) {
    privateKeyPath = await p.text({
      ...
      validate: (value) => {
        ...
        if (!fs.existsSync(expandedPath)) return 'Private key file does not exist';
        return undefined;
      }
    }) as string;
