...
import { EnactCore, type ToolSearchOptions, type ToolExecuteOptions } from "../core/EnactCore";
...
export class EnactDirect {
   ...
   */
  async verifyTool(name: string, policy?: 'permissive' | 'enterprise' | 'paranoid'): Promise<{
    verified: boolean;
    signatures: any[];
    policy: string;
    errors?: string[];
...
export const executeToolByName = (name: string, inputs: Record<string, any> = {}, options: ToolExecuteOptions = {}) => 
...
export const executeRawTool = (toolYaml: string, inputs: Record<string, any> = {}, options: ToolExecuteOptions = {}) => 
...
export const toolExists = (name: string) => 
  enactDirect.toolExists(name);

export const getAuthStatus = () => 
  enactDirect.getAuthStatus();

export const publishTool = (tool: EnactTool) => 
  enactDirect.publishTool(tool);
