...
import LocalToolResolver from './LocalToolResolver';
...
(async () => {
  try {
    ...
    logger.warn(`Failed to initialize LocalToolResolver: ${error instanceof Error ? error.message : String(error)}`);
  }
})();
...
server.registerTool(
  "execute-tool-by-name-enhanced",
  {
    ...
    inputSchema: {
      ...
      localFile: z.boolean().optional().describe("Treat 'name' as a file path to local YAML tool"),
      ...
      forceRegistry: z.boolean().optional().describe("Skip local resolution and go straight to registry")
    }
  },
  async (params) => {
    const { 
    ...
    try {
      logger.info(`Executing tool: ${name} (localFile: ${localFile}, async: ${async})`);
      ...
      if (localFile) {
        ...
        try {
          ...
          toolToExecute = {
            path: resolvedPath,
            definition,
            name: definition.name || path.basename(resolvedPath, path.extname(resolvedPath))
          };
          ...
          return {
            content: [{ type: "text", text: `❌ Local file not found or invalid: ${resolvedPath}` }],
            isError: true
          };
        }
      ...
      const envToValidate = isLocalFile ? toolToExecute.definition?.env : toolToExecute.env;
      if (envToValidate) {
        ...
        if (!envValidation.valid) {
          return {
            content: [{ type: "text", text: envValidation.errorMessage || 'Environment validation failed' }],
            isError: true
          };
        }
      }
      ...
      const isLongRunning = toolToExecute.name.includes('dagger') || 
      ...
      if (isLongRunning) {
        ...
        if (isLocalFile) {
          ...
          executionPromise = enactCore.executeToolByName(toolToExecute.name || name, inputs, {
            timeout: timeout || '300s',
            verifyPolicy, skipVerification, force, dryRun, verbose
          });
        }
...
server.registerTool(
  "manage-local-tools",
  {
    ...
    description: "Manage local tools, aliases, favorites, cache, and view directory structure",
    inputSchema: {
      action: z.enum([
        'list', 'show-directory', 'add-favorite', 'remove-favorite', 
        'add-alias', 'remove-alias', 'cleanup-cache', 'suggestions'
      ...
      switch (action) {
        case 'list':
          ...
          allTools.local.forEach(tool => {
            const relativePath = tool.path.replace(join(homedir(), '.enact', 'tools'), '~/.enact/tools');
        ...
        case 'show-directory':
          ...
          const scanDir = async (dirPath: string, prefix = ''): Promise<string> => {
            let result = '';
            try {
              ...
              for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
                ...
                if (entry.isDirectory()) {
                  ...
                  if (['tools', 'env', 'cache'].includes(entry.name)) {
                    ...
                    result += await scanDir(join(dirPath, entry.name), nextPrefix);
                  }
                } else {
                  let icon = entry.name.endsWith('.yaml') || entry.name.endsWith('.yml') ? '🔧' : 
                            entry.name.endsWith('.env') ? '🔐' : 
                            entry.name.endsWith('.json') ? '⚙️' : '📄';
        ...
        case 'add-favorite':
          ...
          return { content: [{ type: "text", text: `⭐ Added ${toolName} to favorites` }] };
          
        case 'remove-favorite':
          if (!toolName) throw new Error('toolName required for remove-favorite');
          // Add remove method to resolver if needed
          return { content: [{ type: "text", text: `🗑️ Removed ${toolName} from favorites` }] };
          
        case 'add-alias':
          if (!alias || !toolName) throw new Error('Both alias and toolName required');
          toolResolver.addAlias(alias, toolName);
          return { content: [{ type: "text", text: `🔗 Added alias: ${alias} → ${toolName}` }] };
          
        case 'remove-alias':
          if (!alias) throw new Error('alias required for remove-alias');
          // Add remove method to resolver if needed
          return { content: [{ type: "text", text: `🗑️ Removed alias: ${alias}` }] };
          
        case 'cleanup-cache':
          const cleaned = await toolResolver.cleanupCache();
          return { content: [{ type: "text", text: `🧹 Cleaned ${cleaned} expired cache entries` }] };
          
        case 'suggestions':
          if (!partial) throw new Error('partial name required for suggestions');
          const suggestions = await toolResolver.getSuggestions(partial);
          const suggestOutput = suggestions.length > 0 
...
server.registerTool(
  "create-local-tool",
  {
    ...
    inputSchema: {
      ...
      timeout: z.string().optional().describe("Execution timeout"),
      inputSchema: z.record(z.any()).optional().describe("Input schema for the tool"),
      env: z.record(z.any()).optional().describe("Environment variables"),
      force: z.boolean().default(false).describe("Overwrite if tool already exists")
    }
  },
  async ({ name, description, command, tags, timeout, inputSchema, env, force = false }) => {
    try {
      const { promises: fs } = await import('fs');
      const yaml = await import('yaml');
    ...
    try {
      ...
      tools.forEach((tool, index) => {
        ...
        if (detailed) {
          ...
          if (tool.command) {
            const shortCommand = tool.command.length > 50 
...
server.registerTool(
  "check-operation-status",
  {
    ...
    description: "Check the status of background operations. Use 'list' as operationId to see all operations.",
    inputSchema: {
      operationId: z.string().describe("Operation ID to check, or 'list' to show all operations")
    }
  },
  async ({ operationId }) => {
    try {
...
server.registerTool(
  "enact-core-status",
  {
    ...
    inputSchema: {
      detailed: z.boolean().default(false).describe("Show detailed system information")
    }
  },
  async ({ detailed = false }) => {
    try {
      ...
      const activeOps = Array.from(runningOperations.values()).filter(op => op.status === 'running');
